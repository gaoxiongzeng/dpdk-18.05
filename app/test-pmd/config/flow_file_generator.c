...
const int fake_warm_up_src_port = 11110;
const int fake_warm_up_dst_port = 11111;
...
int main(void) 
{
    ...
    for (flow_id=0; flow_id<host_num; flow_id++) {
        flow_size = 1;
        flow_start_time = 0.0;
        ...
        fprintf(output_flow_file, "%d %s %s %d %d %d %.9f\n",
            ...
            host_ip[flow_id],
            fake_warm_up_dst_ip,
            fake_warm_up_src_port,
            fake_warm_up_dst_port,
            flow_size,
            flow_start_time);
        fclose(output_flow_file);
    }

    /* Generate real traffic flows */
